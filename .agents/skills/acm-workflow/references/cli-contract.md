...
- `finish_reason=length` is partial only when coaching produced non-empty readable text: preserve it, emit `done.status=partial`, persist the assistant message as `interrupted`, and include guidance to increase **设置 → 任务路由与费用治理 → 最大输出 Token**. For every task whose output-limit failure leaves no usable result, the API, CLI, background job, and UI expose exactly `输出达到 Token 上限，结果不可用。请在设置中提高对应任务的最大输出 Token。`. Structured artifacts that remain truncated after the single governed repair are unavailable, never complete or apply-ready.
