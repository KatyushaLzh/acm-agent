...
Every Web entry point accepts an actual final Python 3.10 or newer runtime; prereleases and Microsoft Store command aliases do not satisfy the check. Only when no compatible runtime exists does the launcher repeatedly accept `y` or `n`; `n`, EOF, or non-interactive stdin exits without starting the server. On Windows, `y` installs the pinned Python 3.13.15 for the current user without elevation, trying the Huawei Cloud mirror before python.org and verifying the official SHA-256. On supported Linux (glibc or musl on x86_64/ARM64) and macOS (Intel or Apple Silicon), `y` uses uv to install managed Python 3.13.15 under `.acm/runtime`. A matching executable project uv under `.acm/runtime/bootstrap` has priority; if that file is damaged or mismatched, replace only that project file and redownload it. When no project uv exists, reuse a global uv only if its version exactly matches the pin; otherwise leave the global installation and PATH unchanged and download the pinned, checksummed uv into the project. Try the domestic uv mirror first, fall back to GitHub after either a download or SHA-256 failure, and abort Python installation if both sources fail rather than using a mismatched global uv. Python installation tries npmmirror before the official source. The Unix launcher also maintains an interpreter-ABI-and-lock-digest-versioned Web environment there for the pinned system-keyring dependencies; ordinary non-Web CLI commands continue to use the existing `python3` and do not require these packages. Its checked-in lock must be generated from `tools/requirements-web-unix.in` with the pinned uv using universal resolution and Python 3.10 as the lower bound, include every conditional transitive dependency, require hashes and binary distributions at sync time, and import the approved backend before marking an environment ready. This Unix flow must not invoke `sudo`, modify a shell profile or PATH, replace the system Python, or start/unlock a Secret Service; FreeBSD, OpenBSD, Solaris, and other Unix variants are not auto-installed. A dependency-install failure may fall back to a valid base Python for the core Dashboard, but secure credential persistence must report unavailable and retry on a later launch. After installation, require the core Web standard-library modules. A missing `tkinter` does not block the core Dashboard, but native file-picker operations must report that they are unavailable.
...
- Dashboard assistant messages use the same safe Markdown DOM renderer as knowledge-summary previews. Historical assistant messages and every accumulated streaming update must be rendered before display; never expose the raw Markdown source in the chat bubble. Keep user-authored chat messages as plain text, preserve KaTeX rendering, and never use provider Markdown as trusted `innerHTML`.
- A coaching response with `finish_reason=length` is partial only when it contains readable text: preserve the text, persist the assistant message as `interrupted`, keep the provider run auditable with `finish_reason=length`, and direct the user to raise that profile's **最大输出 Token** under **设置 → 任务路由与费用治理**. If any task fails because the output Token limit was reached and no usable result remains, expose exactly `输出达到 Token 上限，结果不可用。请在设置中提高对应任务的最大输出 Token。`. Structured tasks may repair one truncated artifact within the same ledger, but must never cache, apply, or present a still-truncated JSON artifact as complete.
