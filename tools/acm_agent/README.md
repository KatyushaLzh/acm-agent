...
DeepSeek 和 OpenAI-compatible 连接在保存连接或点击“刷新模型”时读取服务端 `/models`，模型 ID 不经过版本白名单过滤。新发现的模型先作为待验证条目显示；首次选择时通过对应任务的 live conformance 后，六个任务 profile 可独立选用。能力和结构化输出接口由模型目录及验证结果决定，不从型号名称推断。离线初始化的旧型号仅用于兼容已有配置，不限制发现新型号；已不在列表中的旧条目保留为不可用，任务不会静默切换到其他模型。`/models` 不提供定价，新型号缺少价格数据时费用显示未知，不套用旧型号价格。 能力验证的文本、JSON 和流式请求共用对应任务的正常输出 Token 上限与超时；未调整配置时使用该任务的默认预算，自动及各思考强度均适用。通用连接能力测试使用推荐任务预算。推理 Token 计入输出预算，截断会记录为 `response_incomplete`，不会作为验证成功。

`gap_fill`（查漏补缺）从不同 AC 题数较少的知识板块选择，`specialization`（专项强化）从不同 AC 题数较多的知识板块选择。两者始终先由确定性引擎生成合规候选，保留当前题数、训练模式、来源模式和题单过滤；网络、鉴权、余额、限流、非法 JSON 或越权题号会优先触发同模式的 hybrid/确定性回退。只有完整本地业务校验通过时回退结果才可用，否则返回结构化 `unavailable`，不会把空或越界集合伪装成成功。
