...
DeepSeek 和 OpenAI-compatible 连接在保存连接或点击“刷新模型”时读取服务端 `/models`，模型 ID 不经过版本白名单过滤。新发现的模型先作为待验证条目显示；首次选择时通过对应任务的 live conformance 后，六个任务 profile 可独立选用。能力和结构化输出接口由模型目录及验证结果决定，不从型号名称推断。离线初始化的旧型号仅用于兼容已有配置，不限制发现新型号；已不在列表中的旧条目保留为不可用，任务不会静默切换到其他模型。`/models` 不提供定价，新型号缺少价格数据时费用显示未知，不套用旧型号价格。
