...
from .stress_runtime import StressCoordinator, StressRuntimeError
...
class AcmService:
    ...
    # ------------------------------------------------------------------
    # Explicit Markdown knowledge summaries
    ...
    def ai_stress_status(self) -> dict[str, Any]:
        ...
        status["settings"] = {
            key: ai["settings"].get(key, default)
            for key, default in DEFAULT_AI_SETTINGS.items()
    ...
    def ai_stress_start(
        ...
        *,
        ...
        if not 0.1 <= float(timeout) <= 60 or not 0.1 <= float(brute_timeout) <= 60:
            ...
            with Database(self.paths.database) as db:
                db.acquire_stress_setup_slot(
                    ...
                    request_summary={
                        ...
                        "brute": bool(generate_brute),
                        "reference": bool(prepare_reference),
                        "large": bool(large_profile),
                        "profile_version": 2,
                        "contains_user_source": False,
                        "source_order": ["official", "cnblogs", "csdn", "ai"],
        ...
        try:
            result = self._stress.start(
                ...
                include_brute=bool(generate_brute),
                include_reference=bool(prepare_reference),
            ...
            with Database(self.paths.database) as db:
                db.update_ai_run(
                    ...
                    status="failed",
                    error={
                        "code": str(getattr(exc, "code", "stress_setup_failed")),
                        "message": str(exc)[:500],
