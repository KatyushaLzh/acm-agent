...
class VerifyTests(unittest.TestCase):
    ...
    def test_verify_samples_when_gpp_available(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            build_dir = (root / ".acm" / "build").resolve()
            self.assertTrue(Path(token_result.compile_command[-1]).resolve().is_relative_to(build_dir))
    ...
    def test_stress_mismatch_preserves_reproduction_assets(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            failure = Path(result.failure_dir or "").resolve()
            failures_dir = (root / ".acm" / "failures").resolve()
            self.assertTrue(failure.is_relative_to(failures_dir))
